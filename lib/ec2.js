...
// disassociateAddress:
// http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/EC2_20130201.html#disassociateAddress-property

exports.disassociate = function (arg,s,f) {
	var iid;
	var ip;
	if (this._item.type == 'ip' ) {
		ip = this._item.id;
	} else {
		ip = arg; // XXX check syntax
	}

	var params = { PublicIp: ip };
	EC2.disassociateAddress( params, function(err,data) {
		//console.log( 'EC2.disassociateAddress:');
		//console.log(data);
		if(err) f(err);
	});

	until_address_associated( ip, undefined, function(err,data) {
		console.log( 'ec2.disassociate success' );
		//console.log(data);
		if(err) f(err);
		else s();
	});
